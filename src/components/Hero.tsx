    ...
    <>
      ...
      >
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          {/* Code bracket - top left, very subtle */}
          ...
          {/* Subtle dots grid - decorative (hidden on mobile) */}
        ...
        <div
          ref={socialsRef}
          className="hidden sm:flex absolute left-3 md:left-6 lg:left-10 top-1/2 -translate-y-1/2 z-20 flex-col items-center gap-2 md:gap-4"
        >
          <div className="w-px h-8 md:h-16 bg-black/20"></div>
          {socials.map((social, index) => (
            <div key={index} className="relative group">
              <a
                ...
                className="w-8 h-8 md:w-10 md:h-10 flex items-center justify-center rounded-full bg-black/10 text-black/70 hover:bg-black hover:text-white transition-all duration-300 hover:scale-110"
                aria-label={social.name}
              >
                {social.icon}
              </a>
              {social.name === "Email" && (
                <div className="hidden lg:block absolute left-14 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-all duration-300 group-hover:translate-x-0 -translate-x-2 z-50">
        ...
        <div className="sm:hidden absolute bottom-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-3">
          {socials.slice(0, 4).map((social, index) => (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="w-9 h-9 flex items-center justify-center rounded-full bg-black/10 text-black/70 hover:bg-black hover:text-white transition-all duration-300"
              aria-label={social.name}
            >
              {social.icon}
            </a>
        ...
        <div className="w-full h-full flex items-center justify-center absolute inset-0 z-5">
          ...
          >
            <div className="mb-1 sm:mb-2">
              <span className="space-grotesk text-sm sm:text-lg md:text-xl lg:text-2xl text-black/60 uppercase tracking-widest">
                Qusai Sakerwala
              </span>
            </div>
          ...
          <div className="hidden md:flex absolute bottom-12 right-10 lg:right-16 z-20 flex-col items-center gap-2 text-black/40">
            <span className="font-mono text-xs uppercase tracking-widest rotate-90 origin-center translate-x-4">
              Scroll
            </span>
            <div className="w-px h-12 bg-black/20 animate-pulse"></div>
          </div>
        </div>
      </div>
    </>
